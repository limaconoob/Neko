...
pub fn command_line() -> Vec<String> {
    ...
    for c in stdin.events() {
        ...
        match b {
            ...
            Event::Key(Key::Left) => {
                if term.curs_x > term.begin_x {
                    term.curs_x -= 1;
                    move_it(0)
                }
            }
            Event::Key(Key::Right) => {
                if term.curs_x < size + term.begin_x {
                    term.curs_x += 1;
                    move_it(1)
                }
            }
            Event::Key(Key::Backspace) => {
                if size > 0 && term.curs_x > term.begin_x {
                    size -= 1;
                    term.curs_x -= 1;
                    move_it(0);
                    buf.remove((term.curs_x - term.begin_x) as usize);
                    let ref mut u: Vec<_> = buf.drain(((term.curs_x - term.begin_x) as usize)..)
                        .collect();
                    let mut j = u.clone();
                    let taille = u.len();
                    for i in u {
