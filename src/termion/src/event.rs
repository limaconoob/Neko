...
pub enum MouseButton {
    ...
    /// Fr    le pointeur
    ShiftCtrlRightDrag, //  **********  AJOUTS D'EVENEMENTS **********
}
    ...
    where I: Iterator<Item = Result<u8, Error>>
{
    ...
    match item {
                ...
                Some(Ok(b'[')) => {
                    match iter.next() {
                        ...
                        Some(Ok(b'M')) => {
                            ...
                            let cx = (iter.next().unwrap().unwrap() as u8)
                                .saturating_sub(32) as u16;
                            let cy = (iter.next().unwrap().unwrap() as u8)
                                .saturating_sub(32) as u16;
        ...
        Ok(c @ b'\x1C'...b'\x1F') => Ok(Event::Key(Key::Ctrl((c as u8 - 0x1C + b'4') as char))),
